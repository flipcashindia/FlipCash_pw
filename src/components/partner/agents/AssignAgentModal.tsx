...
import { useAvailableAgents, useAssignLeadToAgent } from '../../../hooks/useAgents';
import type { AgentProfile } from '../../../api/types/agent.type';
...
const AssignAgentModal: React.FC<AssignAgentModalProps> = ({
  ...
  const { data: availableAgents, isLoading, error: fetchError } = useAvailableAgents(leadId);
  const assignMutation = useAssignLeadToAgent();
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        ...
        >
          ...
          <div className="p-6 overflow-y-auto max-h-[60vh]">
            {/* Error */}
            {(error || fetchError) && (
              <div className="mb-4 p-4 bg-[#FF0000]/10 border-2 border-[#FF0000] rounded-xl flex items-center gap-3">
              ...
              {isLoading ? (
                ...
                <div className="text-center py-12 bg-gray-50 rounded-xl">
                  ...
                  <p className="text-gray-500 text-sm mt-1">
                    {searchQuery ? 'Try different search terms' : 'All agents are currently busy'}
                  </p>
                </div>
              ) : (
            ...
            {selectedAgent && (
              <div className="bg-[#1B8A05]/10 p-4 rounded-xl border-2 border-[#1B8A05]">
                <div className="flex items-center gap-3">
                  <CheckCircle className="text-[#1B8A05]" size={24} />
                  <div className="flex-1">
                    <p className="font-bold text-[#1C1C1B]">Ready to assign</p>
                    <p className="text-sm text-gray-600">
                      {selectedAgent.user.name} will receive this lead assignment
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h4 className="font-bold text-[#1C1C1B] truncate">{agent.user.name}</h4>
            {agent.average_rating && (
              <span className="flex items-center gap-1 text-[#FEC925] text-sm">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-gray-600">
            ...
            <span className="flex items-center gap-1">
              <Activity size={14} />
              {agent.total_visits_completed} completed
            </span>
          </div>
        </div>
